...
VERSIONS='
  ...
  2.6.0:a811db0f1570fb0e95eafe51931e18399d3cefd0f0c7e4074e2a28d4c6dedd66
'
...
# ADIOS1 "make check"; ADIOS2 "make test"
if [ "${VERSION}" == "1.13.1" ]; then
  make --jobs=$make_jobs check || fn_error "tests failed"
else
  make --jobs=$make_jobs test  || fn_error "tests failed"
fi
